...
def retrain():
  iteration_this_run_cycle = 0
  while True:
    current_time = int(time.time())
    if (current_time % 43200) == 0:
      for filename in os.listdir("./CryptoAI/CryptoModelData/PermData/"):
          time.sleep(15)
          cryptos = ["btcusdt", "ethusdt", "dogeusdt"]
          for crypto in cryptos:
            fileData = pd.read_csv(f"./CryptoAI/CryptoModelData/Data/Binance_{crypto.upper()}.csv")
            fileData.to_csv(f"./CryptoAI/CryptoModelData/PermData/Binance_{crypto.upper()}.csv", mode="a", header=None, index=False)

            file_split = filename.split("_")


          # loads data files to a pandas dataframe ---------------------------------->
          df = pd.read_csv(f"./CryptoAI/CryptoModelData/PermData/{filename}", low_memory=False)

          # removing all unnecessary columns (LHC values may be used in the future) ->
          df = df.copy().drop(["Close", "Low", "High", "Volume", "QuoteVolume"], axis=1)
          print(df)
          ...
          # callback for stopping early --------------------------------------------->
          cp_callback = tf.keras.callbacks.ModelCheckpoint(filepath=f"./CryptoModelData/SavedModels/Retrained_{iteration_this_run_cycle}/{file_split[0]}/{file_split[1][:-4]}/Model_Weights", save_weights_only=True, verbose=1)
          ...
          rf.to_csv('./CryptoModelData/Model_results/Results_Final.csv_{iteration_this_run_cycle}/{file_split[0]}/{file_split[1][:-4]}/Model_Weights', mode='a', index=False, header=False)
          ...
          try:
              os.mkdir(f"./CryptoModelData/Model_results/Retrained_{iteration_this_run_cycle}")
          except FileExistsError:
              pass
          os.mkdir(f"./CryptoModelData/Model_results/Retrained_{iteration_this_run_cycle}")
          plt.figure(figsize=(21, 13), dpi=256)
          plt.plot(history.history['loss'], label="Train")
          plt.plot(history.history['val_loss'], label="Validation")
          plt.legend()
          with open(f"./CryptoModelData/Model_results/{file_split[0]}/{file_split[1][:-4]}_Plot/lossGraph.png/Retrained_{iteration_this_run_cycle}", "wb") as f:
              plt.savefig(f)
